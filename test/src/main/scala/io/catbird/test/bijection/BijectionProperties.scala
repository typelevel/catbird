package io.catbird.test.bijection
...
trait BijectionProperties {
  ...
  def isInjective[A, B](implicit a: Arbitrary[A], bij: ImplicitBijection[A, B], eqA: Eq[A]) =
    forAll { (a: A) =>
      eqA.eqv(a, rt(a)(bij.bijection))
    }

  def invertIsInjection[A, B](implicit b: Arbitrary[B], bij: ImplicitBijection[A, B], eqB: Eq[B]) =
    forAll { (b: B) =>
